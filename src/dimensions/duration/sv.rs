...
pub fn rules()->Vec<Rule>{vec![
    ...
    Rule{name:"number.number hours".to_string(),pattern:vec![regex("(\\d+)\\,(\\d+)"),regex("timm(e|ar)?")],production:Box::new(|n|{let m=match &n[0].token_data{TokenData::RegexMatch(m)=>m,_=>return None};let hh:i64=m.group(1)?.parse().ok()?;let frac=m.group(2)?;let num:i64=frac.parse().ok()?;let den:i64=10_i64.pow(frac.len() as u32);Some(TokenData::Duration(DurationData::new(60*hh + (num*60)/den, Grain::Minute)))})},
    Rule{name:"<integer> and an half hours".to_string(),pattern:vec![predicate(is_natural),regex("och (en )?halv timme?")],production:Box::new(|n|{let v=numeral_data(&n[0].token_data)?.value as i64;Some(TokenData::Duration(DurationData::new(30+60*v,Grain::Minute)))})},
