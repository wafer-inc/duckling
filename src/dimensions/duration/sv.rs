...
pub fn rules() -> Vec<Rule> {
    vec![
        Rule {
            ...
            pattern: vec![regex("(1/2|en halv) timme")],
        ...
        Rule {
            ...
            production: Box::new(|n| {
                let v = numeral_data(&n[0].token_data)?.value as i64;
                let g = match &n[1].token_data {
        ...
        Rule {
            ...
            pattern: vec![regex("(\\d+)\\,(\\d+)"), regex("timm(e|ar)?")],
            production: Box::new(|n| {
                let m = match &n[0].token_data {
        ...
        Rule {
            name: "<integer> and an half hours".to_string(),
            pattern: vec![predicate(is_natural), regex("och (en )?halv timme?")],
            production: Box::new(|n| {
                let v = numeral_data(&n[0].token_data)?.value as i64;
                Some(TokenData::Duration(DurationData::new(
        ...
        Rule {
            ...
            pattern: vec![regex("en|ett?"), dim(DimensionKind::TimeGrain)],
            production: Box::new(|n| {
                let g = match &n[1].token_data {
                    TokenData::TimeGrain(g) => *g,
                    _ => return None,
        ...
        Rule {
            ...
            pattern: vec![regex("(precis|exakt)"), dim(DimensionKind::Duration)],
            production: Box::new(|n| Some(n[1].token_data.clone())),
        },
    ]
}
