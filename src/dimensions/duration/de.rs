...
pub fn rules() -> Vec<Rule> {
    vec![
        ...
        Rule {
            ...
            pattern: vec![regex("3/4\\s?stunde|(einer? )?dreiviertel stunde|drei viertelstunden")],
        ...
        Rule {
            ...
            pattern: vec![regex("ungef(ä|a)hr|zirka|genau|exakt"), dim(DimensionKind::Duration)],
        ...
        Rule {
            ...
            pattern: vec![regex("(ein(en)?)?(1/2|halbe?)"), dim(DimensionKind::TimeGrain)],
            production: Box::new(|nodes| {
                let g = match &nodes[1].token_data {
                    TokenData::TimeGrain(g) => *g,
                    _ => return None,
                ...
                match g {
                    Grain::Minute => Some(TokenData::Duration(DurationData::new(30, Grain::Second))),
        ...
        Rule {
            ...
            pattern: vec![dim(DimensionKind::Duration), regex(",|und"), dim(DimensionKind::Duration)],
            production: Box::new(|nodes| {
        ...
        Rule {
            ...
            pattern: vec![
                ...
                predicate(|td| {
                    is_natural(td) && matches!(td, TokenData::Numeral(n) if n.value >= 1.0 && n.value < 60.0)
                }),
                predicate(is_grain(Grain::Minute)),
            ],
            production: Box::new(|nodes| {
