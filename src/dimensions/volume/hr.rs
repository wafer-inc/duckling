...
pub fn rules() -> Vec<Rule> {
    vec![
        ...
        Rule {
            ...
            pattern: vec![regex("m(l|ililita?ra?)")],
        ...
        Rule {
            ...
            pattern: vec![regex("(hektolita?ra?)")],
            production: Box::new(|_| {
                Some(TokenData::Volume(VolumeData::unit_only(
        ...
        Rule {
            ...
            pattern: vec![regex("l(it(a)?r(a|e)?)?")],
            production: Box::new(|_| {
        ...
        Rule {
            name: "half".to_string(),
            pattern: vec![regex("pola"), is_unit_only()],
            production: Box::new(|nodes| {
                let d = volume_data(&nodes[1].token_data)?;
