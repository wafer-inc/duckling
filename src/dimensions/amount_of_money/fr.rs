...
pub fn rules() -> Vec<Rule> {
    vec![
        ...
        Rule {
            name: "£".to_string(),
            pattern: vec![regex("(livre|pound)s?")],
            production: Box::new(|_| {
                Some(TokenData::AmountOfMoney(AmountOfMoneyData::currency_only(
                    Currency::Pound,
                )))
        ...
        Rule {
            name: "unnamed currencyOnly".to_string(),
            pattern: vec![regex("(balle|pouloute)s?")],
            production: Box::new(|_| {
                Some(TokenData::AmountOfMoney(AmountOfMoneyData::currency_only(
                    Currency::Unnamed,
                )))
        ...
        Rule {
            ...
            pattern: vec![regex("plus (que|de)|au moins"), predicate(is_simple_money)],
            production: Box::new(|nodes| {
                let d = money_data(&nodes[1].token_data)?;
                Some(TokenData::AmountOfMoney(
