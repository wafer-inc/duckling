...
fn distance_data(td: &TokenData) -> Option<&DistanceData> {
    if let TokenData::Distance(d) = td {
        Some(d)
    } else {
        None
    }
...
pub fn rules() -> Vec<Rule> {
    vec![
        Rule {
            ...
            pattern: vec![dim(DimensionKind::Distance), regex("k(ilo)?m?(eter)?")],
            production: Box::new(|n| {
                let d = distance_data(&n[0].token_data)?;
        ...
        Rule {
            ...
            pattern: vec![dim(DimensionKind::Distance), regex("cm|centimeter")],
            production: Box::new(|n| {
                let d = distance_data(&n[0].token_data)?;
                Some(TokenData::Distance(
        ...
        Rule {
            name: "<dist> mils".to_string(),
            pattern: vec![dim(DimensionKind::Distance), regex("mils?")],
            production: Box::new(|n| {
                let d = distance_data(&n[0].token_data)?;
                let x = d.value?;
