...
pub fn rules() -> Vec<Rule> {
    ...
    rules.extend(vec![
        ...
        Rule {
            ...
            pattern: vec![regex("(?:الساعة\\s+)?([اأإآء-ي\\d\\s]+?)\\s*(?:الا|إلا)\\s*(ربع|ربعا|ثلث|ثلثا)\\s*(الصبح|هذا الصباح|الفجر|صباحا|صباحًا|قبل الظهر|ظهرا|ظهرًا|بعد الظهر|بعد العصر|عصرا|العصر|مساء|مساءً|ليلا|ليلاً|العشاء|بعد المغرب|قبل المغرب)")],
            production: Box::new(|nodes| {
                let (h, frac, part) = match &nodes[0].token_data {
                    TokenData::RegexMatch(mm) => (mm.group(1)?, mm.group(2)?, mm.group(3)?),
                    _ => return None,
