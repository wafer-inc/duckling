...
fn lex_0_19(s: &str) -> Option<NumeralData> {
    let value = match s {
        ...
        "წყვილი" | "წყვილები" => return Some(NumeralData::new(2.0).with_quantifier()),
        "ცოტა" | "რამდენიმე" | "რამოდენიმე" => return Some(NumeralData::new(3.0).with_quantifier()),
        _ => return None,
    };
...
fn hundreds(s: &str) -> Option<f64> {
    match s {
        ...
        "სამასი" | "სამას" | "სამ ას" | "სამ ასი" => Some(300.0),
        "ოთხასი" | "ოთხას" | "ოთხ ას" | "ოთხ ასი" => Some(400.0),
        "ხუთასი" | "ხუთას" | "ხუთ ას" | "ხუთ ასი" => Some(500.0),
        "ექვსასი" | "ექვსას" | "ექვს ას" | "ექვს ასი" => Some(600.0),
        "შვიდასი" | "შვიდას" | "შვიდ ას" | "შვიდ ასი" => Some(700.0),
        "რვაასი" | "რვაას" | "რვა ას" | "რვა ასი" => Some(800.0),
        "ცხრაასი" | "ცხრაას" | "ცხრა ას" | "ცხრა ასი" => Some(900.0),
        _ => None,
    }
}
...
fn power(s: &str) -> Option<NumeralData> {
    match s {
        "ათასი" | "ათას" => Some(NumeralData::new(1e3).with_grain(3).with_multipliable(true)),
        "მილიონი" | "მილიონ" => {
