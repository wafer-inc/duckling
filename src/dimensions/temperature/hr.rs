...
pub fn rules() -> Vec<Rule> {
    vec![
        ...
        Rule {
            name: "<latent temp> stupnjevi".to_string(),
            pattern: vec![is_value_only(false), regex("deg\\.?|stupa?nj((ev)?a)?|°")],
            production: Box::new(|nodes| {
        ...
        Rule {
            name: "<temp> Celzij".to_string(),
            pattern: vec![is_value_only(true), regex("c(elz?(ija?)?)?\\.?")],
            production: Box::new(|nodes| {
                let td = temperature_data(&nodes[0].token_data)?;
                Some(TokenData::Temperature(
