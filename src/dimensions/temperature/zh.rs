...
pub fn rules() -> Vec<Rule> {
    vec![
        ...
        Rule {
            ...
            pattern: vec![regex("(摄|攝)氏"), is_value_only(true), regex("度|°")],
            production: Box::new(|nodes| {
        ...
        Rule {
            ...
            pattern: vec![regex("(华|華)氏"), is_value_only(true), regex("度|°")],
            production: Box::new(|nodes| {
                let td = temperature_data(&nodes[1].token_data)?;
                Some(TokenData::Temperature(
        ...
        Rule {
            ...
            pattern: vec![is_value_only(true), regex("(摄|攝)氏(°|度)|(°)c")],
            production: Box::new(|nodes| {
                let td = temperature_data(&nodes[0].token_data)?;
                Some(TokenData::Temperature(
                    td.clone().with_unit(TemperatureUnit::Celsius),
        ...
        Rule {
            ...
            pattern: vec![is_value_only(true), regex("(华|華)氏(°|度)|(°)f")],
            production: Box::new(|nodes| {
                let td = temperature_data(&nodes[0].token_data)?;
                Some(TokenData::Temperature(
                    td.clone().with_unit(TemperatureUnit::Fahrenheit),
